...
export default ({ record, card, parentReload }) => {
    ...
    const [gamaOperatoria, setGamaOperatoria] = useState();

    const [modalParameters, setModalParameters] = useState({});
    const [showModal, hideModal] = useModal(({ in: open, onExited }) => (
        <ResponsiveModal lazy={true} footer="ref" onCancel={hideModal} width={1100} height={700}>
            <FormGamaOperatoria forInput={modalParameters.forInput} record={{ ...modalParameters.record }} parentReload={parentReload} />
        </ResponsiveModal>
    ), [modalParameters]);


    useEffect(() => {
        if (record?.gamaoperatoria) {
            setGamaOperatoria(record.gamaoperatoria)
        }
    }, [record.gamaoperatoria]);

    const onEdit = (type) => {
        switch (type) {
            default:
                setModalParameters({ forInput: true, record: { id: record.id, gamaoperatoria: gamaOperatoria } });
                showModal();
                break;
        }
    }
        ...
        <>
            ...
            >
                {Object.keys(record).length > 0 &&
                    <YScroll>
                        {gamaOperatoria && <FormGamaOperatoria record={{ id: record.id, gamaoperatoria: gamaOperatoria }} forInput={false} />}
                    </YScroll>
                }
            </Card>
        </>

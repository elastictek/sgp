...
export default ({ record, card, parentReload }) => {
    ...
    const dataAPI = useDataAPI({ id: "dashb-recicladolist", payload: { url: `${API_URL}/recicladolist/`, parameters: {}, pagination: { enabled: false, limit: 20 }, filter: {}, sort: [{ column: 'timestamp', direction: 'DESC' }] } });
    ...
    const onLoteClick = (row) => {
        navigate('/app/picking/pickreciclado', { state: { id: row.id, level: 0 } });
    }
    ...
    const onView = () => {
        navigate("/app/picking/recicladolist", { state: {} });
    }

        ...
        <React.Fragment key={key}>
            ...
            {filmeEstiravel === 1 &&
                <g transform={`translate(${x + 160},${y})`}>
                    ...
                    <text x="50" y="85" transform="rotate(90, 55, 50)" textAnchor="middle">Filme Estirável</text>
                </g>
            }
        </React.Fragment>
    );
...
export default ({ vGap = 2, form, items, changedValues, x = 200, width = "100%", height, viewBox }) => {
    ...
    useEffect(() => {
        ...
        if (!data.paletizacao) {
            ...
            for (let [i, v] of data.paletizacao.entries()) {
                switch (v.item_id) {
                    case 1:
                        els.push({ obj: SvgPalete, x, height: 25, text: `Palete ${PALETE_SIZES.find(p => p.key == v.item_paletesize).value}`, item_order: v.item_order, item_id: v.item_id });
                        npaletes++;
                        break;
                    case 2:
                        els.push({ obj: SvgBobines, x, height: 100, filmeEstiravel: isValue(data.filmeestiravel_bobines, undefined, 0), bobinesTxt: `${v.item_numbobines} Bobine(s)`, item_order: v.item_order, item_id: v.item_id });
                        break;
                    case 3:
                        els.push({ obj: SvgPlacaCartao, x, height: 15, item_order: v.item_order, item_id: v.item_id });
                        break;
                    case 4:
                        els.push({ obj: SvgPlacaMDF, x, height: 5, item_order: v.item_order, item_id: v.item_id });
                        break;
                    case 5:
                        els.push({ obj: SvgPlacaPlastico, x, height: 10, item_order: v.item_order, item_id: v.item_id });
                        break;
                    case 6:
                        els.push({ obj: SvgCantoneira, x, height: 10, item_order: v.item_order, item_id: v.item_id });
                        break;
                    case 7:
                        els.push({ obj: SvgCutHere, x, height: 10, item_order: v.item_order, item_id: v.item_id });
                        break;
                }
            }
            if (form) {
...
{/*                 
                ...
                {/* <text x="50" y="85" transform="rotate(90, 55, 50)" strokeWidth="2" textAnchor="middle">Filme Estirável</text> */}
            </g>
            }

...
export const loadCortesPlan = async ({ agg_of_id }, signal) => {
    const { data: { rows } } = await fetchPost({ url: `${API_URL}/ordensfabrico/sql/`, filter: { agg_of_id }, sort: [], parameters: { method: "CortesPlanList" }, signal });
...
export default ({ operationsRef, ...props }) => {
    ...
    const [showModal, hideModal] = useModal(({ in: open, onExited }) => {
        const content = () => {
            switch (modalParameters.content) {
                // case "cortes": return <Chooser parameters={modalParameters.parameters} />;
    ...
    //     const _filter = {};
    //     setModalParameters({
    //         content: "cortes", responsive: true, type: "drawer", width: "85%", title: "Cortes disponíveis", push: false, loadData: () => { }, parameters: {
    //             offsetHeight: "200px",
    ...
    const onGenerateCortes = () => {
        setModalParameters({ content: "generate", type: "drawer", width: "95%", title: "Gerar Cortes", lazy: true, push: false/* , loadData: () => dataAPI.fetchPost() */, onSelectPlan, permissions: permission.permissions, parameters: { ...pickAll(["agg_of_id"], inputParameters.current),data:dataAPI.getData().rows } });
        showModal();
    }
    ...
    const loadData = async ({ signal, init = false } = {}) => {
        submitting.trigger();
        if (init) {
            const { tstamp, ...paramsIn } = loadInit({}, { ...dataAPI.getAllFilter(), tstamp: dataAPI.getTimeStamp() }, props?.parameters, location?.state, ["temp_ofabrico_agg", "agg_of_id"]);
            inputParameters.current = { ...pickAll(["temp_ofabrico_agg", "agg_of_id"], paramsIn) };
        }
        const _rows = await loadCortesPlan({ agg_of_id: inputParameters.current.agg_of_id }, signal);
    ...
    const columns = [
        ...
        ...(true) ? [{ name: 'start', header: 'Início', headerAlign: "center", userSelect: true, defaultLocked: true, defaultWidth: 80 }] : [],
        ...(true) ? [{ name: 'iterations', header: 'Bobinagens', headerAlign: "center", userSelect: true, defaultLocked: true, defaultWidth: 80 }] : [],
        ...(true) ? [{ name: 'cortes', flex: 1, header: 'Cortes', render: ({ data, cellProps }) => <div style={{ color: "#1890ff", fontWeight: 600 }}>{data.cortes.replaceAll('"', ' ')}</div> }] : [],
        ...(true) ? [{ name: 'cortes_ordem', flex: 1, header: 'Posicionamento', render: ({ data, cellProps }) => <div style={{ color: "#1890ff", fontWeight: 600 }}>{data.cortes_ordem.replaceAll('"', ' ')}</div> }] : [],
        ...(true) ? [{ name: 'largura_util', header: 'Largura Útil', headerAlign: "center", userSelect: true, defaultLocked: true, defaultWidth: 80 }] : [],
    ...
    const onSelectPlan = (rows) => {
        console.log(rows?.cortes)
        if (rows?.cortes && Array.isArray(rows?.cortes)) {
            let acc = 1;
            const _rows = rows?.cortes.map((r, idx) => {
                let v = { id: r?.idx, idx: idx + 1, cortes: JSON.stringify(r?.n_cortes), cortes_ordem: JSON.stringify(r?.cortes_ordem), iterations: r?.n, start: acc, largura_util:r?.largura_util };
                acc = acc + r.n;
                return v;
            });
            dataAPI.setData({ rows: _rows, total: _rows.length });
    ...
    const onSave = async () => {
        let response = null;
        try {
            response = await fetchPost({ url: `${API_URL}/ordensfabrico/sql/`, filter: { ...pickAll(["agg_of_id"], inputParameters.current) }, parameters: { method: "SaveCortesPlan", rows: dataAPI.getData().rows } });
    ...
    return (
        <>
            ...
            <Container fluid>
                        ...
                        <YScroll>
                            ...
                            <Table
                                ...
                                leftToolbar={<Space style={{ alignSelf: "center" }}>
                                    <Permissions permissions={permission} {...PERMISSION} forInput={[mode.datagrid.edit, props?.editParameters?.isEditable(false)]}><Button onClick={onGenerateCortes}>Gerar Cortes</Button></Permissions>
                                    {/* <Permissions permissions={permission} {...PERMISSION} forInput={[mode.datagrid.edit, props?.editParameters?.isEditable(false)]}><Button onClick={onSelectCorte}>Adicionar Cortes</Button></Permissions> */}
                                </Space>}
                ...
                {(operationsRef && props?.activeTab == '7') && <Portal elId={operationsRef.current}>

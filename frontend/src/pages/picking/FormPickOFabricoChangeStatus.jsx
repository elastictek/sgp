                                    ...
                                    <Row>
                                        {state.step == 0 && <Col><AggChoose openNotification={openNotification} actionsContent={(item, index, load, allowInit) =>
                                            [
                                                ...
                                                ...[2,3].includes(item.items[0].ofabrico_status) ? [<Button key={`B3-${index}`} type='primary' onClick={() => onChange(item.items[0], 9, load)} danger icon={<FaStop size={12} />}>Finalizar Produção</Button>] : []
                                            ]

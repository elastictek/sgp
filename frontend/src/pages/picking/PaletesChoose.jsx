...
export default ({ extraRef, closeSelf, loadParentData, noid = true, defaultFilters = {}, defaultSort = [], onSelect, title, onFilterChange, ...props }) => {
    ...
    const columns = [
        ...(true) ? [{ name: 'nome', header: 'Nome', filter: { show: "toolbar", op: "any", alias: "lote" }, userSelect: true, defaultLocked: true, defaultWidth: 120, headerAlign: "center", render: ({ data, cellProps }) => <LeftAlign style={{ fontWeight: "700" }}>{data?.nome}</LeftAlign> }] : [],
        ...
        ...(true) ? [{ name: "nbobines_real", header: "Bobines", filter: { show: true, type: "number", alias: "nbobinesreal", field: { style: { width: "70px" } } }, defaultWidth: 80, userSelect: true, defaultlocked: false, headerAlign: "center", render: ({ data, cellProps }) => <LeftAlign style={{}}>{String(data.nbobines_real).padStart(2, '0')}/{String(data.num_bobines).padStart(2, '0')}</LeftAlign> }] : [],
        ...(true) ? [{ name: "estado", header: "Estado", filter: { show: true, alias: "estados", type: "selectmulti", field: { style: { width: "80px" }, keyField: 'value', textField: 'value', options: BOBINE_ESTADOS } }, defaultWidth: 90, userSelect: true, defaultlocked: false, headerAlign: "center", render: ({ data, cellProps }) => <EstadoBobines id={data.id} nome={data.nome} artigos={json(data.artigo)} /> }] : [],
        ...(true) ? [{ name: "largura", header: "Largura", filter: { show: true, type: "number", field: { style: { width: "90px" } } }, defaultWidth: 120, userSelect: true, defaultlocked: false, headerAlign: "center", render: ({ data, cellProps }) => <Largura id={data.id} nome={data.nome} artigos={json(data.artigo)} /> }] : [],
        ...(true) ? [{ name: 'ofid', header: 'Ordem Fabrico', filter: { show: "toolbar", op: "any", alias: "of" }, userSelect: true, defaultLocked: false, defaultWidth: 120, headerAlign: "center", render: ({ data, cellProps }) => <OF id={data.id} ofid={data.ofid} of_des={data.ordem_original} /> }] : [],
        ...(true) ? [{ name: "prf", header: "Prf", filter: { show: true, op: "any" }, defaultWidth: 130, userSelect: true, defaultlocked: false, headerAlign: "center", render: ({ data, cellProps }) => <LeftAlign style={{}}>{data.prf}</LeftAlign> }] : [],
        ...(true) ? [{ name: "iorder", header: "Encomenda", filter: { show: true, op: "any", alias: "order" }, defaultWidth: 130, userSelect: true, defaultlocked: false, headerAlign: "center", render: ({ data, cellProps }) => <LeftAlign style={{}}>{data.iorder}</LeftAlign> }] : [],
        ...(true) ? [{ name: "cliente_nome", header: "Cliente", filter: { show: true, op: "any", alias: "cliente" }, defaultWidth: 230, flex: 1, userSelect: true, defaultlocked: false, headerAlign: "center", render: ({ data, cellProps }) => <LeftAlign style={{}}>{data.cliente_nome}</LeftAlign> }] : [],
        ...(true) ? [{ name: "destino", header: "Destino", filter: { show: true, op: "any", alias: "destinoold" }, defaultWidth: 230, userSelect: true, defaultlocked: false, headerAlign: "center", render: ({ data, cellProps }) => <LeftAlign style={{}}>{data.destino}</LeftAlign> }] : [],
    ];

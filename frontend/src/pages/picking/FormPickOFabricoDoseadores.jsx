...
const TitleForm = ({ level, auth, hasEntries, onSave, loading, title, subTitle }) => {
    return (<ToolbarTitle id={auth?.user} description={`${title}/${subTitle}`} details={<span style={{ fontSize: "16px", marginLeft: "70px" }}>{subTitle}</span>}
...
export default ({ extraRef, closeSelf, loadParentData, ...props }) => {
    ...
    const [subTitle, setSubTitle] = useState("Gerir Doseadores");
                                    ...
                                    <Row>
                                        ...
                                        {state.step == 1 && <Col>
                                            <YScroll maxHeight={"80vh"}>
                                                <FormFormulacao noHeader={true} setFormTitle={true} parameters={{ cs_id: state.pos.items[0].cs_id, type: "formulacao_dosers_change" }} />
                                            </YScroll>
                                        </Col>}
                                    </Row>
                                </Container>

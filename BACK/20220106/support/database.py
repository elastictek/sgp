...
def replace(regex, s, data):
    ...
    for value in re.finditer(regex, s):
        ...
        start = newstart
    newstring += s[start:]
    return {"filter": newstring, "values": values if len(values) > 1 else list(values.values())[0]}
...
class BaseSql:
    ...
    def __computeSort(self, dictData):
        sort = {"column": None, "field": None, "direction": "ASC", **dictData}
        ...
        if column:
            return f"""{column} {sort["direction"]}"""
        return ""
...
class PostgresSql(BaseSql):
    ...
    def dql(self, data, computeColumns=True):
    ...
    def dml(self, typeDml, data, table=None, returning=None, encloseColumns=True):
        ...
        if typeDml == TypeDml.DELETE:
            if table is not None:
                if returning is None:
                    ret.statement = f'DELTE FROM {table}'
                else:
                    ret.statement = f'DELTE FROM {table} returning {returning}'
            return ret
        ...
        if typeDml == TypeDml.INSERT:
            ...
            if table is not None:
                if returning is None:
                    ret.statement = f'UPDATE {table} SET {",".join(ret.tags)}'
                else:
                    ret.statement = f'UPDATE {table} SET {",".join(ret.tags)} returning {returning}'
        return ret

    def count(self, table, p={}, connOrCursor=None):
        ret = BaseSql.Count()
        f = Filters(p)
        f.where()
        f.auto()
    ...
    def exists(self, table, p={}, connOrCursor=None):

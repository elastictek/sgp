...
'''
def solve_large_model(demands, parent_width=100, max_cutters=None):
  num_orders = len(demands)
  iter = 0
  ...
  while iter < 50:
    status, y, l = solve_master(patterns, quantities, parent_width=parent_width, integer=False)
    iter += 1

    # list widths of orders
    widths = [demands[i][1] for i in range(num_orders)]
    new_pattern, objectiveValue = get_new_pattern(l, widths, parent_width=parent_width, max_cutters=max_cutters)
...
def solve_master(patterns, quantities, parent_width=100, integer=False):
  ...
  y = [ solver.IntVar(0, 10000, '') for j in range(n) ] # right bound?
...
def get_new_pattern(l, w, parent_width=100, max_cutters=None):
  ...
  if max_cutters is not None:
    solver.Add( sum( 1 * new_pattern[i] for i in range(n)) <= max_cutters ) 
...
def get_initial_patterns(demands):
  num_orders = len(demands)
  return [[0 if j != i else 1 for j in range(num_orders)] for i in range(num_orders)]
...
'''
    params
        ...
        parent_rolls: 
            ...
            e.g.: [ [quantity, width], [quantity, width], ...]
'''
def StockCutter1D(child_rolls, parent_rolls, output_json=True, large_model=True, max_cutters=None):
  ...
  if not large_model:
    ...
    status, A, y, consumed_big_rolls = solve_large_model(demands=child_rolls, parent_width=parent_width, max_cutters=max_cutters)
...
if __name__ == '__main__':
  ...
  def main(infile_name: Optional[str] = typer.Argument(None)):
    ...
    child_rolls = [[384, 130],[384, 65]]
    parent_rolls = [[10, 2100]] # 10 doesn't matter, itls not used at the moment
    max_cutters = 24

    #child_rolls = [[84,150],[324, 120]]
    #parent_rolls = [[10, 2100]] # 10 doesn't matter, itls not used at the moment

    ...
    def saveItems(data,cursor):
        if "rows" in data:
            for idx, item in enumerate(data.get("rows")):
                args = (data.get("cs_id"),None,item["artigo_cod"],item["artigo_des"],item["vcr_num"],item["n_lote"],item["qty_lote"],item["largura"],item["comp"],item["lote_id"],item["pos"],0,request.user.id,0)
